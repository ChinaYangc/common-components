 ...
 * satisfy Postel's principle: <blockquote> be conservative in what you do, be liberal in what you accept from others
 * </blockquote>
 * <p/>
 * Applied to JSON-like content from others, it will produce well-formed JSON that should satisfy any parser you use.
 * <p/>
 * Applied to your output before you send, it will coerce minor mistakes in encoding and make it easier to embed your
 * JSON in HTML and XML.
 * <p/>
 * <h3>Input</h3> The sanitizer takes JSON like content, and interprets it as JS eval would. Specifically, it deals with
 * these non-standard constructs.
 * <ul>
 ...
 * </ul>
 * <p/>
 * The sanitizer fixes missing punctuation, end quotes, and mismatched or missing close brackets. If an input contains
 * only white-space then the valid JSON string {@code null} is substituted.
 * <p/>
 * <h3>Output</h3> The output is well-formed JSON as defined by <a href="http://www.ietf.org/rfc/rfc4627.txt">RFC
 * 4627</a>. The output satisfies three additional properties:
 * <ol>
 * <li>The output will not contain the substring (case-insensitively) {@code "&lt;/script"} so can be embedded inside an
 * HTML script element without further encoding.
 * <li>The output will not contain the substring {@code "]]>"} so can be embedded inside an XML CDATA section without
 * further encoding.</li>
 * <li>The output is a valid Javascript expression, so can be parsed by Javascript's <code>eval</code> builtin (after
 * being wrapped in parentheses) or by <code>JSON.parse</code>. Specifically, the output will not contain any string
 * literals with embedded JS newlines (U+2028 Paragraph separator or U+2029 Line separator).
 * <li>The output contains only valid Unicode scalar values (no isolated UTF-16 surrogates) that are <a
 * href="http://www.w3.org/TR/xml/#charsets">allowed in XML</a> unescaped.
 * </ol>
 * <p/>
 * <h3>Security</h3> Since the output is well-formed JSON, passing it to <code>eval</code> will have no side-effects and
 * no free variables, so is neither a code-injection vector, nor a vector for exfiltration of secrets.
 * <p/>
 * <p>
 * This library only ensures that the JSON string &rarr; Javascript object phase has no side effects and resolves no
 * free variables, and cannot control how other client side code later interprets the resulting Javascript object. So if
 * client-side code takes a part of the parsed data that is controlled by an attacker and passes it back through a
 * powerful interpreter like {@code eval} or {@code innerHTML} then that client-side code might suffer unintended
 * side-effects.
 * <p/>
 * <h3>Efficiency</h3> The sanitize method will return the input string without allocating a new buffer when the input
 * is already valid JSON that satisfies the properties above. Thus, if used on input that is usually well formed, it has
...
public final class JsonSanitizer {
    // log
    private static final Logger LOG = LoggerFactory.getLogger(JsonSanitizer.class.getName());

    /**
     * 将字符串变成有效的json字符串
     * 
     * @param jsonish 需要转换的json字符串
     * @return 有效的json字符串
     */
    ...
    /**
     * Describes where we are in a state machine that consists of transitions on complete values, colons, commas, and
     * brackets.
     */
    private enum State {
        /**
         * Immediately after '[' and {@link #BEFORE_ELEMENT before the first element}.
         */
        START_ARRAY,
        /**
        ...
        BEFORE_ELEMENT,
        /**
         * After a JSON value in an array or at the top level, and before any following comma or close bracket.
         */
        AFTER_ELEMENT,
        /**
        ...
        BEFORE_VALUE,
        /**
         * After a value in a key-value map but before any following comma or close bracket.
         */
        AFTER_VALUE;
    }
    ...
    /**
     * The number of brackets that have been entered and not subsequently exited. Also, the length of the used prefix of
     * {@link #isMap}.
     */
    private int bracketDepth;

    /**
     * {@code isMap[i]} when {@code 0 <= i && i < bracketDepth} is true iff the i-th open bracket was a <code>'{'</code>
     * , not a <code>'['</code>.
     */
    private boolean[] isMap;

    /**
     * If non-null, then contains the sanitized form of {@code jsonish.substring(0, cleaned)}. If {@code null}, then no
     * unclean constructs have been found in {@code jsonish} yet.
     */
    private StringBuilder sanitizedJson;

    /**
     * The length of the prefix of {@link #jsonish} that has been written onto {@link #sanitizedJson}.
     */
    private int cleaned;

    private static final int NEGATIVE_NUM = -6;
            ...
            try {
                ...
                switch (ch) {
                    ...
                    case '[':
                        ...
                        if (isMap == null) {
                            final int booleanCapacity = 64;
                            isMap = new boolean[booleanCapacity];
                        }
                        boolean map = ch == '{';
                    ...
                    default:
                        ...
                        for (runEnd = i; runEnd < n; ++runEnd) {
                            ...
                            if (('a' <= tch && tch <= 'z') || ('0' <= tch && tch <= '9') || tch == '+' || tch == '-'
                                    || tch == '.' || ('A' <= tch && tch <= 'Z') || tch == '_' || tch == '$') {
                                continue;
                            }
                            break;
                        }
    ...
    /**
     * Ensures that the output corresponding to {@code jsonish[start:end]} is a valid JSON string that has the same
     * meaning when parsed by Javascript {@code eval}.
     * <ul>
     * <li>Making sure that it is fully quoted with double-quotes.
     * <li>Escaping any Javascript newlines : CR, LF, U+2028, U+2029
     * <li>Escaping HTML special characters to allow it to be safely embedded in HTML {@code <script>} elements and XML
     * {@code <!CDATA[...]]>} sections.
     * <li>Rewrite hex, octal, and other escapes that are valid in Javascript but not in JSON.
     * </ul>
     *
            ...
            // Fixup newlines.
                ...
                case '/':
                    ...
                    if (i > start && i + 2 < end && '<' == jsonish.charAt(i - 1)
                            && 's' == (jsonish.charAt(i + 1) | NUM_32) && 'c' == (jsonish.charAt(i + 2) | NUM_32)) {
                        insert(i, '\\');
                    }
                    break;
                case ']':
                ...
                case '\\':
                    ...
                    switch (sch) {
                        ...
                        case 'v': // Recognized by JS but not by JSON.
                            ...
                            break;
                        case 'x':
                            if (i + NUM_4 < end && isHexAt(i + 2) && isHexAt(i + NUM_3)) {
                            ...
                            break;
                        case 'u':
                            if (i + NUM_6 < end && isHexAt(i + 2) && isHexAt(i + NUM_3) && isHexAt(i + NUM_4)
                                    && isHexAt(i + NUM_5)) {
                                i += NUM_5;
                                break;
                            }
                        ...
                        case '7':
                            ...
                            if (octalEnd + 1 < end && isOctAt(octalEnd + 1)) {
                                ...
                                for (int j = i; j < octalEnd; ++j) {
                                    value = (value << NUM_3) | (jsonish.charAt(j) - '0');
                                }
                ...
                default:
                    ...
                    // | [#x10000-#x10FFFF]
                    final int char0x20 = 0x20;
                    final int char0xa = 0xa;
                    final int char0xd = 0xd;
                    final int char0xd800 = 0xd800;
                    final int char0xe000 = 0xe000;
                    final int char0xfffd = 0xfffd;
                    final int char0xf = 0xf;
                    if (ch < char0x20) {
                        if (ch == NUM_9 || ch == char0xa || ch == char0xd) {
                            continue;
                        }
                    }
                    else if (ch < char0xd800) { // Not a surrogate.
                        continue;
                    }
                    else if (ch < char0xe000) { // A surrogate
                    ...
                    else if (ch <= char0xfffd) { // Not one of the 0xff.. controls.
                        continue;
                    }
                    replace(i, i + 1, "\\u");
                    for (int j = NUM_4; --j >= 0;) {
                        sanitizedJson.append(HEX_DIGITS[(ch >>> (j << 2)) & char0xf]);
                    }
                    break;
            }
        }
    ...
    private void elide(int start, int end) {
        if (sanitizedJson == null) {
            sanitizedJson = new StringBuilder(jsonish.length() + NUM_16);
        }
    ...
    /**
     * The position past the last character within the quotes of the quoted string starting at {@code s.charAt(start)}.
     * Does not assume that the quoted string is properly closed.
     */
        ...
        for (int i = closeBracketPos; --i >= cleaned;) {
            switch (jsonish.charAt(i)) {
                ...
                case ',':
                    ...
                    return;
                default:
                    throw new AssertionError("" + jsonish.charAt(i));
            }
        }
    ...
    /**
     * Ensures that the given run of characters is a valid JSON number. This is less aggressive than
     * {@link #canonicalizeNumber} since it can be called on inputs that are valid JSON so is on the fast path.
     * <p/>
    ...
    private void normalizeNumber(int start, int end) {
        ...
        if (pos == intEnd) { // No empty integer parts allowed in JSON.
            ...
            if (intEnd - pos == 1 && intEnd < end && 'x' == (jsonish.charAt(intEnd) | NUM_32)) { // Recode hex.
                int value = 0;
                ...
                for (int i = pos; i < intEnd; ++i) {
                    value = (value << NUM_3) | (jsonish.charAt(i) - '0');
                }
                elide(pos, intEnd);
                sanitizedJson.append(value);
            }
        ...
        // Optional exponent.
        if (pos < end && 'e' == (jsonish.charAt(pos) | NUM_32)) {
            ++pos;
            if (pos < end) {
                switch (jsonish.charAt(pos)) {
    ...
    /**
     * Converts a run of characters that form a JS number to its canonical form which happens to also be a valid JSON
     * number.
     */
    ...
    private static void canonicalizeNumber(StringBuilder sanitizedJson, int sanStart, int sanEnd) {
        ...
        if (fractionEnd == sanEnd) {
            ...
            assert 'e' == (sanitizedJson.charAt(fractionEnd) | NUM_32);
        ...
        int exp = expEnd == expStart ? 0 : Integer.parseInt(sanitizedJson.substring(expStart, expEnd));
    ...
    private void appendHex(int x, int nDigits) {
        final int char0xf = 0xf;
        for (int i = 0; i < nDigits; ++i, x >>>= NUM_4) {
            int dig = x & char0xf;
            sanitizedJson.append(dig + (dig < NUM_10 ? '0' : (char)('a' - NUM_10)));
        }
    }
    ...
    /**
     * An exception instance that can be thrown without the cost of unwinding the stack.
     */
